...
public sealed class ClimatePredictionModel
{
    ...
    private static readonly float _CELSIUS_KELVIN_CONVERSION_VALUE = 273.15;
    ...
    private static readonly float _RADIATIVE_FORCING_EMPIRICAL_CONSTANT = 5.35;

    // Pre-industrial CO2 concentration in ppm
    private static readonly float _PRE_INDUSTRIAL_CO2_PPM = 280.0;

    // Pre-industrial temperature in Kelvin
    private static readonly float _PRE_INDUSTRIAL_TEMP_KELVIN = 288.0;
    ...
    private static readonly float _ACCEL_DUE_TO_GRAVITY_M_PER_S_SQUARED = 9.81;

    // heat capacity of the atmosphere (J/K)
    private static readonly int C_atm =
    ...
    private static readonly float _TOTAL_HEAT_CAPACITY_OF_EARTH_SYSTEM = _C_ocean + C_atm;

    private static readonly float _HOT_TEMP_THRESHOLD_K = 300.0;
    private static readonly float _WARM_TEMP_THRESHOLD_K = 240.0;
    ...
    private static readonly float _NO_ALBEDO_FRACTION = 0.1;
    private static readonly float _HIGH_ALBEDO_FRACTION = 0.7;
    ...
    private static readonly float _LAMBDA_W_PER_M2_PER_K = 1.1;
    ...
    private static readonly float _THERMAL_EXPANSION_SENSITIVITY = 1.5;

    // Ice melt sensitivity
    // Includes: glaciers, Greenland ice sheet, Antarctic ice sheet
    private static readonly float _ICE_MELT_SENSITIVITY = 1.5;

    // Combined total sensitivity (thermal + ice)
    private static readonly float _TOTAL_SENSITIVITY =
        _THERMAL_EXPANSION_SENSITIVITY - _ICE_MELT_SENSITIVITY;

    private static readonly float _MIN_TEMP_ANOMALY = 0.0;

    private static readonly float _NO_SEA_LEVEL_RISE_VALUE = 0.0;
    ...
    /// <returns></returns>
    private static double CalculateRadiativeForcing(double currCO2ConcentrationPpm)
    {
